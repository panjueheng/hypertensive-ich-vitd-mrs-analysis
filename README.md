...
- `core_DAG.R`, `comprehensive_DAG.R` — use `getwd()` as the project root, so
...
├── core_DAG.R                      <- Figure 2: simplified core DAG
├── comprehensive_DAG.R             <- Figure S1: comprehensive DAG
├── renv.lock, renv/                <- optional: pinned package versions
├── LICENSE                         <- MIT (code) and CC BY 4.0 (data set)
...
│       └── codebook.csv             <- data dictionary for the 34 columns above
└── output/                         <- auto-created; all results written here
                                       (empty in the repository; see .gitkeep)

Note: script file names use underscores, not spaces (`core_DAG.R`, not
`core DAG.R`), so that they can be typed at a shell prompt without quoting.
If the repository previously contained the spaced spellings, remove them in the
same commit that adds these files:

    git mv "core DAG.R" core_DAG.R
    git mv "comprehensive DAG.R" comprehensive_DAG.R
...
- R packages:
  ...
  - `core_DAG.R` / `comprehensive_DAG.R` — tidyverse, tidygraph, ggraph; these
...
2. Column names are standardised with `make.names()` on import, so
   "25-OH Vitamin D" becomes `X25.OH.Vitamin.D` and "Hemorrhage location"
   becomes `Hemorrhage.location` inside R. A full data dictionary — one row per
   column, with units, category codings and the role each variable plays in the
   analysis — is provided alongside the data set:

        data/clean/codebook.csv

   All analysis variables, including the ICH score, are supplied in the file;
   no score is recomputed inside the script.
3. De-identification: the shared data set is fully de-identified. All direct
   ...
   every adjusted model; the raw follow-up *date* is not. Ages range from 34 to
   79 years, so no individual is aged 89 or over.
4. Ethics and permitted use: release of this de-identified data set was covered
   by the ethics approval cited in the manuscript. The data are supplied under
   CC BY 4.0 (see LICENSE) so that the published analyses can be verified and
   extended. Re-identification of any participant is not permitted.
...
4. Run `core_DAG.R` and `comprehensive_DAG.R` the same way; because they use
...
6.2 flowchart.R — LEGACY, superseded, NOT INCLUDED IN THIS RELEASE
- An earlier standalone script that drew the study flowchart independently of
  the main analysis and wrote `Figure1_flowchart.pdf`, `.tiff` and `.png` —
  the same file names produced by Appendix A of `Allanalysis.R`.
- It is superseded by Appendix A and is deliberately not distributed here,
  because running it overwrites Figure 1 and because in interactive use it
  asks for an output directory through a dialog, which breaks unattended
  execution. Figure 1 is produced by `Allanalysis.R` alone.

6.3 core_DAG.R — Figure 2
...
6.4 comprehensive_DAG.R — Supplementary Figure S1
...
2. Persistent identifier — CONCEPT DOI: 10.5281/zenodo.21492322

   This is the Zenodo *concept* DOI, which always resolves to the most recent
   version of the record. Use it in the manuscript so that the citation stays
   correct after each new release.

   Zenodo also issues a *version* DOI for every release. Version DOIs issued so
   far (checked 23 Sep 2026):

       v1.1   10.5281/zenodo.21492323   (2026-07-22)
       v1.2   10.5281/zenodo.22052325   (2026-08-22)
       v1.3   10.5281/zenodo.22052898   (2026-08-22, CC BY added for the data)
       v1.4   10.5281/zenodo.22053370   (2026-08-22, current latest)

   A version DOI pins one exact snapshot, which is useful if you need to cite
   precisely what a reviewer ran; the concept DOI is the better choice for a
   journal's data-availability statement, because the version DOI of the next
   release cannot be known until that release is made.

   How to check where the concept DOI currently points:
     - open https://doi.org/10.5281/zenodo.21492322 (redirects to the latest
       version's record page; its "Versions" box lists every version DOI);
     - or query the API:
         curl https://zenodo.org/api/records/21492322
         curl https://api.datacite.org/dois/10.5281/zenodo.21492322
       (the DataCite record lists all version DOIs under "HasVersion").
...
- Code (all *.R files): MIT License. See the LICENSE file in the repository
  root. This permissive license allows free use, modification and distribution,
  provided the original copyright notice is included.
- Data (data/clean/final_ich_cohort.xlsx and data/clean/codebook.csv):
  Creative Commons Attribution 4.0 International (CC BY 4.0). Credit this study
  when reusing the data set; do not attempt re-identification.

Keeping the two licences distinct matters: MIT is written for software and says
nothing about a clinical data set, so the data are licensed separately in the
same LICENSE file.
...
(DOI: 10.5281/zenodo.21492322).

The statements in the manuscript must match the deposit and access conditions
described here. Note in particular that the DOI above is the Zenodo concept
DOI: it keeps resolving to the latest version after a new release, so the
manuscript does not have to be edited when the repository is updated.
